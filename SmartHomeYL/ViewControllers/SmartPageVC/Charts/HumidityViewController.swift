...
class HumidityViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        bt.frame = CGRect(x: 0, y: 10, width: self.view.bounds.width,
                          height: 40)
        bt.setTitle("室内湿度变化统计表", for: .normal)
        ...
        token = result.observe({ (changes: RealmCollectionChange) in
            ...
            case .initial:
                ...
                debugPrint("湿度表初始化")
                if items.count == 0 || itemstate[0].humiState == false{
                    //创建折线图组件对象
                    ...
                    //折线图无数据时显示的提示文字
                    self.chartView.noDataText = "暂无湿度数据"
                    ...
                    //向表内添加数据
                    for item in items {
                        let entry = ChartDataEntry.init(x: Double(i), y: Double(item.humidity))
                        dataEntries.append(entry)
                        times.append(item.time)
                        i += 1
                    }
                    let chartDataSet = LineChartDataSet(values: dataEntries, label: "湿度")
                    //目前折线图只包括1根折线
                    ...
                    //界限1
                    let limitLine1 = ChartLimitLine(limit: 39, label: "🔥")
                    limitLine1.lineWidth = 1 //线宽
                    self.chartView.leftAxis.addLimitLine(limitLine1)
                    limitLine1.lineColor = UIColor(r: 255, g: 69, b: 0) //颜色
                    limitLine1.labelPosition = .rightBottom //位置
                    limitLine1.lineDashLengths = [4, 2] //虚线样式
                    //界限2
                    let limitLine2 = ChartLimitLine(limit: 81, label: "💧")
                    limitLine2.lineWidth = 1 //线宽
                    limitLine2.lineColor = UIColor(r: 0, g: 191, b: 255) //颜色
